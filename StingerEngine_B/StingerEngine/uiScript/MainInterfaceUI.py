...
from StingerEngine.include.clientTools import NotifyMsg, compGame, logger
...
class MainInterfaceUI(ScreenNode):
	...
	def Create(self):
		try:
			...
			logger.error("MainInterfaceUI Create出错:\n{}".format(errinfo))
			try:
				self.SetRemove()
			except Exception:
				pass
	...
	def OnOpenLoadPanel(self, args):
		...
		EngineClient.RequestSaveList(MAIN_INTERFACE_SOURCE)

	def OnCloseLoadPanel(self, args):
		self._hide_load_modal()

	def OnSettings(self, args):
		EngineClient.CreateSystemMenuUI("settings", "title")
	...
	def OnLoadSlotSelected(self, slot_id, slot_meta):
		...
		EngineClient.RequestSaveLoad(slot_id, MAIN_INTERFACE_SOURCE)
	...
	def _handle_load_response(self, data):
		...
		self.SetRemove()
		EngineClient.CreateGameUI(snapshot.get("entry"), "resume", slot_id, snapshot)
